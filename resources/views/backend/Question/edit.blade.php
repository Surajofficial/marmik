    ...
    <div class="card-body">
      <form method="post" action="{{route('testimonial.update',$testimonial->id)}}">
        ...
        <div class="form-group">
          ...
          <input id="inputTitle" type="text" name="title" placeholder="Enter title"  value="{{$testimonial->title}}" class="form-control">
          @error('title')
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" id="quote" name="quote">{{$testimonial->quote}}</textarea>
          @error('quote')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <select name="added_by" class="form-control">
              ...
              @foreach($users as $key=>$data)
                <option value='{{$data->id}}' {{(($testimonial->added_by==$data->id)? 'selected' : '')}}>{{$data->name}}</option>
              @endforeach
          </select>
        </div>
        <div class="form-group">

...
@section('title','Dr Awish || CTA Create')

@section('main-content')

<div class="card">
    <h5 class="card-header">Add CTA</h5>
    <div class="card-body">
      <form method="post" action="{{route('cta.store')}}">
        ...
        <input id="inputTitle" type="text" name="url" placeholder="Enter title"  value="{{old('url')}}" class="form-control">
        @error('url')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>

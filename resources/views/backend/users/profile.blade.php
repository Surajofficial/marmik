                ...
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                          ...
                          <select name="role" class="form-control" disabled="disabled">
                              <option value="">-----Select Role-----</option>
                                  <option value="admin" {{(($profile->role=='admin')? 'selected' : '')}}  disabled="disabled">Admin</option>
                                  <option value="user" {{(($profile->role=='user')? 'selected' : '')}}  disabled="disabled">User</option>
                                  <option value="admin" {{(($profile->role=='branch')? 'selected' : '')}}  disabled="disabled">Branch</option>
                                  <option value="user" {{(($profile->role=='employee')? 'selected' : '')}}  disabled="disabled">Employee</option>
                          </select>
                        @error('role')
                        <span class="text-danger">{{$message}}</span>
                        @enderror

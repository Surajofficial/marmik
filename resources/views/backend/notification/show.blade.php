<div id="notifications">
    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown"
        ...
        <span class="badge badge-danger badge-counter">
            @if (count(Auth::guard('admin')->user()->unreadNotifications) > 5)
                <span data-count="5" class="count">5+</span>
            @else
                <span class="count"
                    data-count="{{ count(Auth::guard('admin')->user()->unreadNotifications) }}">{{ count(Auth::guard('admin')->user()->unreadNotifications) }}</span>
            @endif
        </span>
    </a>
    <!-- Dropdown - Alerts -->
